...
public class SelectionSort {
    ...
    private String nomeArquivo = "756491_selection.txt";
    private String nMatricula = "756491/787869";
    private long nInicioExecucao = 0;
    private int nComparacoes = 0;
    private int nMovimentacoes = 0;
    public static SelectionSort programa = new SelectionSort();

    public static void main(String[] args) {
        programa.nInicioExecucao = System.currentTimeMillis();
        Scanner scan = new Scanner(System.in);
        List<SelectionSort> arrayAcomodacoes = new ArrayList<>();
        Map<String, SelectionSort> acomodacoes = new HashMap<>();
        ...
        // );
        // arrayAcomodacoes = quickSort(arrayAcomodacoes, 0, arrayAcomodacoes.size() -
        // 1);
        // arrayAcomodacoes = heapSort(arrayAcomodacoes);

        for (SelectionSort acomodacao : arrayAcomodacoes) {
    ...
    public SelectionSort(int roomId, int hostId, String roomType, String country, String city, String neighbourhood,
            int reviews,
    ...
    public static Map<String, SelectionSort> readFile() {
        Scanner scan = new Scanner(System.in);
        String arquivoOrigem = "/tmp/dados_airbnb.txt";
        // String arquivoOrigem =
        // "C:\\Users\\joao.aguiar\\Desktop\\AEDSII\\AEDSII_2024\\Ordenação\\arquivo.txt";
        ...
        try {
            ...
            while ((linha = leitor.readLine()) != null) {
                if (!linha.isEmpty()) {
                    ...
                    SelectionSort registro = new SelectionSort(Integer.parseInt(parametros[0].trim()),
                            Integer.parseInt(parametros[1].trim()), parametros[2].trim(), parametros[3].trim(),
                            parametros[4].trim(), parametros[5].trim(),
                            Integer.parseInt(parametros[6].trim()), Double.parseDouble(parametros[7].trim()),
                            Integer.parseInt(parametros[8].trim()), Double.parseDouble(parametros[9].trim()),
    ...
    public static void printLine(SelectionSort registro) {
        System.out.println("[" + registro.getRoomId() + " ## " + registro.getHostId() + " ## " + registro.getRoomType()
                + " ## " + registro.getCountry() + " ## " + registro.getCity() + " ## " +
                registro.getNeighbourhood() + " ## " + registro.getReviews() + " ## "
                + registro.getOverallSatisfaction() + " ## " + registro.getAccommodates() + " ## " +
    ...
    @Override
    public SelectionSort clone() {
        try {
            SelectionSort clone = (SelectionSort) super.clone();
    ...
    public static List<SelectionSort> selectionSort(List<SelectionSort> acomodacaos) {
        for (int i = 0; i <= acomodacaos.size() - 1; i++) {
            int min_index = i;
            int min_value = 0;
            for (int j = i + 1; j < acomodacaos.size(); j++) {
                programa.nComparacoes++;
                if ((min_value = acomodacaos.get(j).getCountry()
                        .compareTo(acomodacaos.get(min_index).getCountry())) != 0) {
                    programa.nComparacoes++;
                    if (min_value < 0) {
                        min_index = j;
                    }
                } else if ((min_value = acomodacaos.get(j).getCity()
                        .compareTo(acomodacaos.get(min_index).getCity())) != 0) {
                    programa.nComparacoes += 2;
                    if (min_value < 0) {
                        min_index = j;
                    }
                } else if ((min_value = acomodacaos.get(j).getNeighbourhood()
                        .compareTo(acomodacaos.get(min_index).getNeighbourhood())) != 0) {
            ...
            if (acomodacaos.get(i).getRoomId() != acomodacaos.get(min_index).getRoomId()) {
                ...
                SelectionSort temp = acomodacaos.get(i);
    ...
    public static List<SelectionSort> bubbleSort(List<SelectionSort> acomodacaos) {
        for (int i = 0; i < acomodacaos.size() - 1; i++) {
            for (int j = 0; j < acomodacaos.size() - i - 1; j++) {
                if (acomodacaos.get(j).getOverallSatisfaction() > acomodacaos.get(j + 1).getOverallSatisfaction() ||
                    ...
                    SelectionSort temp = acomodacaos.get(j);
    ...
    public static List<SelectionSort> insertionSort(List<SelectionSort> acomodacoes) {
        for (int i = 1; i < acomodacoes.size(); i++) {
            SelectionSort chave = acomodacoes.get(i);
            int j = i - 1;
            ...
            programa.nComparacoes++;
            while (j >= 0 && acomodacoes.get(j).getAccommodates() == chave.getAccommodates()
                    && acomodacoes.get(j).getRoomId() > chave.getRoomId()) {
                programa.nComparacoes++;
                programa.nMovimentacoes++;
                acomodacoes.set(j + 1, acomodacoes.get(j));
                j = j - 1;
    ...
    public static List<SelectionSort> mergeSort(List<SelectionSort> array, int inicio, int fim) {
        programa.nComparacoes++;
        if (inicio < fim) {
    ...
    private static void intercalar(List<SelectionSort> acomodacoes, int inicio, int meio, int fim) {
        List<SelectionSort> arrayTemp = new ArrayList<>(acomodacoes);
    ...
    public static List<SelectionSort> quickSort(List<SelectionSort> acomodacoes, int esquerda, int direita) {
        programa.nComparacoes++;
        if (esquerda < direita) {
    ...
    private static int separar(List<SelectionSort> acomodacoes, int esquerda, int direita) {
        SelectionSort pivo = acomodacoes.get(esquerda);
        int i = esquerda + 1;
        int j = direita;
    ...
    private static int compararAcomodacoes(SelectionSort acomodacao1, SelectionSort acomodacao2) {
        programa.nComparacoes++;
    ...
    public static List<SelectionSort> heapSort(List<SelectionSort> acomodacoes) {
    ...
    public static void maxHeap(List<SelectionSort> acomodacoes, int n, int i) {
        int maior = i;
        int esquerda = 2 * i + 1;
        int direita = 2 * i + 2;
    ...
    public static int compararAcomodacoesHeap(SelectionSort acomodacao1, SelectionSort acomodacao2) {
        programa.nComparacoes++;
    ...
    public static void createLogFile() {
        String content = programa.nMatricula + "\t"
                + Long.toString(System.currentTimeMillis() - programa.nInicioExecucao) + "\t" + programa.nComparacoes
                + "\t" + programa.nMovimentacoes;
        try {
